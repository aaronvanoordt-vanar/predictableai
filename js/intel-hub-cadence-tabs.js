...
(function () {
  ...
  // ────────── Orquestador desde el browser (1 seccion por vez) ──────────
  // Resuelve el WORKER_RESOURCE_LIMIT de Supabase: en vez de pedir las 9 secciones
  // en una sola llamada (que dura demasiado), pedimos el plan y disparamos
  // una sección a la vez, con sleep entre cada una.
  async function generateAll({ force = false } = {}) {
    ...
    console.log('[intel-cadence] requesting plan…');
    const planResp = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + session.access_token },
      body: JSON.stringify({ force, plan_only: true }),
    });
    const plan = (await planResp.json()).plan || [];
    const toRun = plan.filter((p) => !p.skip).map((p) => p.section_key);
    console.log(`[intel-cadence] plan: ${toRun.length} secciones a correr, ${plan.length - toRun.length} skip por cadencia`);

    for (const section_key of toRun) {
      console.log(`[intel-cadence] ▶ ${section_key}`);
      try {
        ...
        console.log(`[intel-cadence] ${r.status}`, j);
      } catch (e) {
        console.error(`[intel-cadence] ${section_key} failed`, e);
      }
      // Sleep 1.5s entre secciones para no chocar rate limit a nivel API key
      await new Promise((res) => setTimeout(res, 1500));
    }
    console.log('[intel-cadence] all sections done. Reloading reports…');
    await loadReports();
  }

  // Debug + acción manual
  window.intelCadenceReload = () => Promise.all([loadReports(), loadFeedback(), loadLearning()]);
  window.intelCadenceGenerate = generateAll;

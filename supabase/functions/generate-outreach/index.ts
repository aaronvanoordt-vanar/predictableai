...
Deno.serve(async (req: Request) => {
  ...
  let body: { lead?: Lead; sender?: Sender; member_id?: string; engine?: string };
  ...
  try {
    ...
    console.log(`[outreach] ✓ ${user.id} via ${engine}/${generatedVia} (brief:${brief?.status ?? "none"}, hub:${hubReports?.length ?? 0}, snapshot:${snapshot ? "yes" : "no"})`);

...
- Key tables: `profiles` (incl. `role`), `sales_reports`, `intelligence_hub_reports`, `client_icp`, `client_brief`, `prospect_lists`/`prospect_list_members` (client-writable, owner-scoped RLS), `client_sheet_state`/`client_crm_rows`/`client_metric_snapshots`/`client_reviews` (solo la service role escribe; `authenticated` solo lee vía `can_view_client`), credits. Roles: admin / director / SDR.

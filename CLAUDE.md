...
- **Campañas omnicanal = `js/campaigns.js` (lista, detalle, enrolar, bandeja de revisión) + `js/campaign-builder.js` (asistente de 4 pasos con la línea de tiempo gráfica; no toca la base, devuelve el borrador por `onSave`) + edge functions `campaign-run` (motor, pg_cron cada minuto, con JWT: lo llama la service role), `generate-campaign` (cadencia recomendada por IA, 6 créditos), `channel-connect` (conectar WATI/Dripify) y `wati-webhook` (público, `--no-verify-jwt`).** Diseño, decisiones y hallazgos de las APIs en `docs/OMNICANAL.md`. **La cadencia que ejecuta el motor es el grafo `campaigns.flow`** (acciones + condiciones con ramas Sí/No, espera relativa al paso anterior): esquema, validación y recorrido en `supabase/functions/_shared/campaign-flow.ts` ↔ `js/campaign-flow.js`, **espejos exactos que se cambian juntos en el mismo PR** (`deno test` cubre el TS). Regla de parada fija: una respuesta por cualquier canal, la baja o la detención manual. El mensaje IA de cada paso vive en `campaign_messages` (solo el motor inserta; el cliente edita texto y aprueba) y lo genera `generate-outreach` en modo `step`. Plan y decisiones del builder en `docs/CAMPAIGN_BUILDER_PLAN.md`; `campaign_steps` ya no existe (se borró en la Entrega 2) — no la resucites. **WhatsApp sale SOLO por WATI** (cada usuario conecta su tenant): la integración directa con la Cloud API de Meta (`js/whatsapp-inbox.js`, `whatsapp-send/webhook/followups`, tablas `whatsapp_*`) se eliminó a propósito el 2026-09-02 — no la resucites. Email de campaña = mensajes individuales por Apollo (no secuencias). LinkedIn = Dripify (PR 2): la Open API solo permite **subir leads a una campaña ya creada en Dripify** (`POST /campaigns/{id}/leads`), no envía mensajes ni fija campos por lead, así que el paso `linkedin_connect` enrola al lead en la campaña elegida (`settings.dripify_campaign_id` del nodo), `campaign-run` sincroniza el estado cada 15 min leyendo los leads de Dripify, y `dripify-webhook` (público, `--no-verify-jwt`, `?key=` por cuenta; el usuario pega la URL en cada campaña de Dripify) recibe respuestas. La personalización de 5 capas para LinkedIn se entrega como CSV para Custom Lead Fields de Dripify.

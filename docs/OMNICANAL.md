...
- **Con la key compartida no se crean contactos en Apollo (2026-09-23)**: `apollo-proxy` responde `POST /contacts` con `{ contact: null }` y limpia los `contacts` de `/mixed_people/api_search` (quedan como personas, sin email ni etiquetas). Esa cuenta es de todos: lo que un cliente guardaba ahí, otro lo veía como «Guardado» con el email revelado. Con Apollo propio (OAuth o API key) todo sigue igual. Los `apollo_contact_id` que ya estaban guardados eran de la cuenta compartida: `channel-connect` los olvida al conectar un Apollo distinto del anterior (o el primero), y se recrean en la cuenta nueva al enviar.

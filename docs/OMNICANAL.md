...
| Mensajes de WhatsApp | Tres plantillas de saludo por usuario, con su nombre y cargo escritos dentro (los botones de Meta no admiten variables): saludo, recordatorio, último intento. Solo el nombre del lead es variable. Botones: "Hola {Nombre}! Qué tal?" y "Darse de baja". El mensaje de 5 capas va como texto libre cuando el lead responde (ventana de 24 h). |
